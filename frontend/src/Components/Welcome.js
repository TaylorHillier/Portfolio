...
function WelcomePage({ apiUrl }) {
  ...
  return (
    ...
    <section className='welcome m-auto h-screen flex justify-center items-center sticky inset-0 z-10' id='welcome'>
      <div>
        <div className='absolute top-20 right-0 left-0 z-10 m-4'>
          <Logo />
        </div>
        <ParticleBg />
          <div className='welcomeContent w-[80vw] mt-0 m-auto md:mx-auto backdrop-brightness-90 shadow-lg shadow-slate-400'>
            <div className='welcome-content-container p-4'>
              {homeFields.acf?.home_page_repeater?.map((item, index) => (
                <div key={index}>
                  <p className='py-2 mt-4'>{item.text_area_1}</p>
                  ...
                  <div className='buttons flex flex-col my-8 gap-4' id='cta-buttons'>
                    <button className='border-solid p-2 font-bold text-[#0a0a19] bg-white hover:bg-transparent hover:text-white hover:border-white hover:border'>
                      <a href='#projects'>{item.first_cta}</a>
                    </button>
                    <button id='secondCTA' className=' border-solid text-white border-white border-2 p-2 hover:bg-white hover:text-[#0a0a19] hover:font-bold'>
